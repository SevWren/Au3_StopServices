...
Global $sFolder = @ProgramFilesDir & "\StopServicesProcess"
Global $sLogFile = $sFolder & "\closure_log.txt"
; Ensure the directory exists before writing the log
If Not FileExists($sFolder) Then
    If Not DirCreate($sFolder) Then
        MsgBox($MB_SYSTEMMODAL, "Error", "Failed to create directory: " & $sFolder)
        Exit
    EndIf
EndIf
...
Func LogClosure($sName, $sType) ; Function to log the date and time of closure
    ...
    Local $sLogEntry = @CRLF & $sDateTime & " - " & $sType & " '" & $sName & "' was closed." & @CRLF

    ; Attempt to write to the log file
    If Not FileWrite($sLogFile, $sLogEntry) Then
        MsgBox($MB_SYSTEMMODAL, "Error", "Failed to write to log file: " & $sLogFile)
    EndIf
EndFunc   ;==>LogClosure
